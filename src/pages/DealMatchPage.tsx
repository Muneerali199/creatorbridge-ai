...
import { creators, type Niche, type FollowerTier, getFollowerTier } from '../data/creators';
...
export default function DealMatchPage() {
  ...
  const filters = useMemo(
    () => ({ niches, minBudget, maxBudget, tiers }),
    [niches, minBudget, maxBudget, tiers]
  );
  ...
  const toggleTier = (t: FollowerTier) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: '#9b8ec7' }}>
          AI-ranked creator recommendations with auto-generated deal briefs.
        </p>
      </div>
      ...
      <div className="glass-panel p-4 rounded-xl">
        <div className="flex items-center gap-2 mb-4">
          ...
          {(niches.length > 0 || tiers.length > 0 || minBudget > 0) && (
            <button
              onClick={() => { setNiches([]); setTiers([]); setMinBudget(0); setMaxBudget(500000); }}
              className="ml-auto text-xs text-[#ff6b8a] hover:text-[#ff6b8a]/80 flex items-center gap-1"
            >
              ...
              {allNiches.map((n) => (
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-full transition-all ${
                    niches.includes(n)
                      ? 'text-white'
                      : 'text-[#9b8ec7] hover:text-[#f0e6ff]'
                  }`}
                  style={niches.includes(n) ? { background: '#ff6b8a' } : { background: '#2a2348' }}
                >
                  {n}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="card-surface px-3 py-2 text-xs text-[#f0e6ff] outline-none w-24"
                placeholder="Min"
              />
            ...
            <div className="flex flex-wrap gap-1.5">
              {allTiers.map((t) => (
                <button
                  ...
                  style={tiers.includes(t) ? { background: '#34d399', color: '#0a0718' } : { background: '#2a2348' }}
                >
                  {t}
                </button>
              ))}
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 card-surface p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#9b8ec7]">
              Ranked Results
            </h3>
          ...
          <div className="space-y-1 max-h-[600px] overflow-y-auto pr-2">
            {results.map((r, i) => (
              <CreatorCard
                ...
                rank={i + 1}
                selected={selectedCreatorId === r.creator.id}
                onClick={() => setSelectedCreatorId(r.creator.id)}
              />
            ))}
          </div>
        </div>
            ...
            {selectedResult && selectedValue && selectedTier ? (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,107,138,0.15)' }}>
                    <FileText className="w-5 h-5 text-[#ff6b8a]" />
                  </div>
                ...
                <div>
                  ...
                  <div className="text-3xl font-bold text-[#ff6b8a]">
                    ₹{selectedValue.basePrice.toLocaleString()}
                  </div>
                  <span className="text-xs text-[#9b8ec7]">
                    Range: ₹{selectedValue.rangeLow.toLocaleString()} - ₹{selectedValue.rangeHigh.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="space-y-3 pt-3" style={{ borderTop: '1px solid #2a2348' }}>
                  {[
                    ...
                    <div key={item.label}>
                      ...
                      <p className="text-sm font-medium text-[#f0e6ff] mt-0.5">{item.value}</p>
                    </div>
                  ))}
                </div>

                <button className="w-full py-3 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90" style={{ background: '#ff6b8a' }}>
                  Generate Contract
                </button>
              </motion.div>
            ) : (
              <div className="card-surface p-8 text-center">
                <FileText className="w-12 h-12 text-[#2a2348] mx-auto mb-4" />
                <p className="text-sm text-[#9b8ec7]">Select a creator to view their deal brief</p>
              </div>
            )}
          </AnimatePresence>
        </div>
